    ...
    evidence_rows = [
        {
            ...
            "captured_at": _format_datetime(item.get("capturedAt"))
            if isinstance(item.get("capturedAt"), datetime)
            else _row_value(item, "capturedAt"),
        }
...
def generate_member_evaluation_sheet_docx(
    ...
    """Generate BM-MTEC-NS-03 member evaluation sheet DOCX from evaluation report data."""
    context = _evaluation_sheet_context(report, actor)
    template_path = _first_existing_template(MEMBER_EVALUATION_TEMPLATE_PATHS)
    if template_path:
